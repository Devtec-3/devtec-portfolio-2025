...
export async function sendContactEmails(data: ContactFormData): Promise<void> {
  try {
    ...
    console.log('SendGrid client ready, sending from:', fromEmail);
    ...
    console.log('Sending notification to owner:', OWNER_EMAIL);
    await client.send(notificationToOwner);
    console.log('✓ Notification email sent to owner');

    console.log('Sending confirmation to sender:', data.email);
    await client.send(confirmationToSender);
